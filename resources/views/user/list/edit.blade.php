...
@section('content')
    ...
    @elseif ($post->publish_flg === 1)
        <div class="bg-green-200 text-center text-green-700 font-bold">ステータス：公開</div>
    ...
    <form action="{{ route('post.update', ['post_id' => $post->id]) }}" method="post" class="p-5">
        ...
        @if ($errors->any())
            <div class="flex shadow-lg rounded-sm">
                ...
                <div
                    class="px-4 py-6 bg-red-50 rounded-r-lg justify-between items-center border w-full border-l-tranceparent border-red-50">
        ...
        <div class="flex mt-6 mr-12">
            <div class="m-auto">
                <select name="category"
                    class="block w-64 text-gray-700 py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500">
                    ...
                    @foreach ($categories as $category)
                        ...
                        @elseif ($category->id == $post->category_id)
                            <option value="{{ $category->id }}" @selected($post->category_id)>{{ $category->category_name }}
                            </option>
        ...
        <div class="w-full mt-4">
            <input type="text" name="title" class="block w-full text-3xl font-bold border-none outline-none"
                placeholder="タイトルを追加" value="{{ old('title', $post->title) }}">
        </div>
        <div class="w-full mt-4">
            <textarea name="body"
